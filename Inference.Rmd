...
ward_simulation <- function(P_hosp, P_ICU_pre, P_Hd){
...
icu_simulation <- function(P_ICUd, P_ICU_Postd){
...
ABC_acceptance <- function(par){
  ...
  ward_sim = ward_simulation(par[1], par[2], par[3])
  icu_sim = icu_simulation(par[4],par[5])
...
run_ABC <- function(startvalue, iterations){
...
num_iter = 10000
start_time <- Sys.time()
chain <- run_ABC(c(P_hosp,P_ICU_pre, P_Hd, P_ICUd, P_ICU_Postd),num_iter)
end_time <- Sys.time()
...
```{r}
hist(chain[1:num_iter, 1], nclass=30, main="Posterior of P_hosp")
hist(chain[1:num_iter, 2], nclass=30, main="Posterior of P_ICU_pre")
hist(chain[1:num_iter, 3], nclass=30, main="Posterior of P_Hd")
hist(chain[1:num_iter, 4], nclass=30, main="Posterior of P_ICUd")
hist(chain[1:num_iter, 5], nclass=30, main="Posterior of P_ICU_Postd")
```
# Obtain new transition parameters
```{r}
new_Ph = chain[num_iter,1]
new_PICUpre = chain[num_iter,2]
new_PHd = chain[num_iter,3]
new_PICUd = chain[num_iter,4]
new_PICUpd = chain[num_iter,5]
...
# Visualization
```{r}
new_ward_simulation = ward_simulation(new_Ph, new_PICUpre, new_PHd)
new_icu_simulation = icu_simulation(new_PICUd, new_PICUpd)
```

```{r}
new_hosp_simulation = 
  new_ward_simulation[[1]] +
  new_icu_simulation[[1]] +
  new_icu_simulation[[2]] +
  new_icu_simulation[[3]]
```
...
hosp_occupancy <- ggplot() + 
  ...
  scale_colour_manual("", 
                      ...
                      values = c("blue", "black")) +
  labs(x = "Date (Year: 2021)", title = "Hospital Occupancy Simulation (after inference) in VIC") +
  scale_y_continuous("Hospital Occupancy", limits = c(0,900))
hosp_occupancy
```
...
icu_occupancy <- ggplot() + 
  ...
  scale_colour_manual("", 
                      breaks = c("simulation", "data"),
                      values = c("blue", "black")) +
  labs(x = "Date (Year: 2021)", title = "ICU Occupancy Simulation (after inference) in VIC") +
  scale_y_continuous("ICU Occupancy", limits = c(0,250))
icu_occupancy
```


```{r}
new_death_simulation = 
  new_ward_simulation[[2]] +
  new_icu_simulation[[4]] +
  new_icu_simulation[[5]]
```
...
daily_death_plot <- ggplot() + 
  ...
  scale_colour_manual("", 
                      breaks = c("simulation", "data"),
                      values = c("blue", "black")) +
  labs(x = "Date (Year: 2021)", title = "Daily Death Simulation (after inference) in VIC") +
...
ggarrange(hosp_occupancy,icu_occupancy, daily_death_plot, 
          ncol = 1, nrow = 3)
ggsave("Plots/Simulation after inference in VIC.png")
```
